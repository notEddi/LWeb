...
// Verifica che l'ID della recensione sia passato
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errori)) {
        // Salva direttamente il testo dalla textarea, senza nl2br
        $stmt_aggiorna = $conn->prepare(
            "UPDATE " . T_RECENSIONI . " 
             SET voto = ?, testo = ?, data_recensione = NOW() 
...
<head>
    ...
    <title>Modifica Recensione - <?php echo htmlspecialchars($recensione['titolo']); ?></title>
                ...
                <div class="window-body">
                    <?php if (!empty($errori)): ?>
                        <div style="color: red; margin-bottom: 10px;">
                            <?php foreach ($errori as $errore) echo "<p>$errore</p>"; ?>
                        </div>
                    <?php endif; ?>
                    ...
                    <form action="modificaRecensione.php?id=<?php echo $recensione_id; ?>" method="POST">
                        ...
                        <select name="voto" id="voto" required>
                            ...
                            <?php for($i=5; $i>=1; $i--): ?>
                                <option value="<?php echo $i; ?>" <?php echo $recensione['voto']==$i?'selected':''; ?>>
                                    <?php echo $i; ?>
                                </option>
                            <?php endfor; ?>
                        </select>
                        ...
                        <div class="field-row-stacked" style="margin-top: 10px;">
                            ...
                            <textarea id="testo" name="testo" rows="6" required class="textarea-win98"><?php 
                                // Converte eventuali \r\n in \n per la textarea
